...
export default function Dashboard() {
  ...
  const loadChartData = () => {
    apiFetch(`/api/metrics-data/?metric_type_id=${selectedMetric}`)
      .then(res => res.json())
      .then(data => {
  ...
  const changeStatus = async (reportId, newStatusId) => {
    try {
      const res = await apiFetch(`/api/reports/${reportId}/change_status/`, {
        ...
        body: JSON.stringify({ status_id: newStatusId ? parseInt(newStatusId) : null })
      })
      if (res.ok) loadData()
    } catch (err) {}
  }

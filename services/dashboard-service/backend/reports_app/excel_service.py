        ...
        for i, row in enumerate(rows):
            try:
                ...
                if created:
                    ...
                    property_obj.district = row.get('район') or row.get('district', '')
                    property_obj.property_type = row.get('тип_недвижимости') or row.get('property_type', 'жилое')
                    property_obj.total_area = float(row.get('площадь', 0)) if row.get('площадь') else None
                    property_obj.status = row.get('статус') or row.get('status', 'эксплуатируется')
                    property_obj.renovation_status = row.get('статус_реновации') or row.get('renovation_status', '')
                    property_obj.build_year = int(row.get('год_постройки')) if row.get('год_постройки') else None
                    property_obj.floors = int(row.get('этажность')) if row.get('этажность') else None
                    property_obj.save()
                    imported += 1

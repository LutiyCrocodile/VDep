"""Обработка пользовательского превью (кадр 320×180, JPEG, crop cover) без OpenCV."""
...
def _crop_center_cover(img: Image.Image, target_w: int, target_h: int) -> Image.Image:
    """Обрезка по центру под соотношение сторон (как CSS object-fit: cover)."""
...
def process_custom_thumbnail(image_bytes: bytes) -> bytes:
    """Заполнить кадр 16:9 без чёрных полос, вернуть JPEG."""
    try:

...
# Подряд идущие опросы MediaMTX без публикатора (защита от ложного завершения эфира).
...
async def _promote_stream_live_on_rtmp(
    db: AsyncSession,
    stream: Stream,
    *,
    notify: bool = True,
...
async def _reconcile_stream_rtmp_state(
    ...
    if publishing is True:
        ...
        if not stream.is_live:
            ...
            return await _promote_stream_live_on_rtmp(db, stream)
        return stream
